...
httpRequest customHeaders: [[name: 'PRIVATE-TOKEN', value: "${GIT_TOKEN}"]], httpMode: 'POST', url: "${GIT_URL}${PPL_Project_ID}/statuses/${commitId}?state=failed"
...
httpRequest customHeaders: [[name: 'PRIVATE-TOKEN', value: "${GIT_TOKEN}"]], httpMode: 'POST', url: "${GIT_URL}${PPL_Project_ID}/statuses/${commitId}?state=success"
          }
   ...
   stage 'Create  Package'
    ...
    stage ' Publish Package'
            ...
            while (System.currentTimeMillis() < end_time_5 && status_5 != 200 )
        {
            try {
                         ...
                         nexusArtifactUploader  artifactId: "${artifactId}",
                           ...
                           file: "target/${artifactId}-dist.zip",
                           groupId: "com/skilrock/ppl",
     ...
     def response_5 = httpRequest  timeout: 30, url: "http://${NEXUS_BASE_URL}/repository/PPL/com/skilrock/ppl/${artifactId}/${releaseVersion}/${artifactId}-${releaseVersion}.zip", validResponseCodes: '200'
            ...
            sleep(1)
              ...
              slackSend color: "#439FE0", message: "Build Number:${BUILD_NUMBER} Deployment Started For: ${BRANCH_NAME} For Weaver",  teamDomain: 'skilrock', token: 'PJZoglka3a5W4T6fZUfLwWfE'
                echo 'Deploying'
       build job: 'TomcatTestJob', parameters: [string(name: 'target', value: "ppl"), string(name: 'NEXUS_URL', value: "${NEXUS_BASE_URL}"), string(name: 'RepoName', value: "PPL"), string(name: 'GroupId', value: "com/skilrock/ppl"), string(name: 'ArtifactId', value: "${artifactId}"), string(name: 'ReleaseVersion', value: "${releaseVersion}"), string(name: 'FileName', value: "${artifactId}-${releaseVersion}.zip"),string(name: 'version', value: "0.0.1")]
